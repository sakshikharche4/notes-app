...
const App = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-linear-to-br from-gray-900 via-black to-gray-800 text-white p-10">
      {/* Left form section */}
      <form
      ...
      <div className="lg:w-1/2 p-10 border-t-2 lg:border-t-0 lg:border-l-2 border-gray-700 overflow-y-auto">
        ...
        <div className="flex flex-wrap gap-5">
          {task.map((elem, index) => (
            <div
              key={index}
              className="relative bg-white text-black rounded-2xl p-5 w-64 h-64 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 hover:-translate-y-1"
            >
              <button
              ...
              <p className="mt-3 text-gray-700 font-medium leading-snug">
                {elem.detail}
              </p>
            </div>
          ))}
